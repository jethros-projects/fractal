...
final class FocusLifecycleE2ETests: XCTestCase {
    func testE2ECompleteSingleBlockAndPersistHistory() async {
        await MainActor.run {
    ...
    func testE2EContinueRestartsSameTopicAndLogsTwoBlocks() async {
        await MainActor.run {
    ...
    func testE2EContinueCanPrepareNextBlockWithoutAutoStarting() async {
        await MainActor.run {
    ...
    func testE2ESwitchStartsNewTopicAndLogsDistinctSessions() async {
        await MainActor.run {
    ...
    func testE2ESwitchCanSetTopicWithoutStartingNextBlock() async {
        await MainActor.run {
    ...
    func testE2ELogOnlyKeepsCompletedSessionAndStopsThere() async {
        await MainActor.run {
            let rig = makeRig(blockLengthMinutes: 15)
    ...
    func testE2EPauseResumeCompletesOnlyAfterRemainingActiveTime() async {
        await MainActor.run {
    ...
    func testE2EHistoryTotalsUpdateAcrossTodayAndThisWeek() async {
        await MainActor.run {
    ...
    func testE2EBlankTopicIsSavedAsNoTopicSession() async {
        await MainActor.run {
            let rig = makeRig(blockLengthMinutes: 5)
